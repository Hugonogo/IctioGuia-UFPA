...
def treinar_e_salvar_modelo(X, y, label_encoders, le_y, features, model_file='modelo_arvore.pkl'):
    num_amostras = len(X)
...
def carregar_modelo(model_file='modelo_arvore.pkl'):
    if not os.path.exists(model_file):
        raise FileNotFoundError(f"Arquivo do modelo {model_file} não encontrado.")
    data = joblib.load(model_file)
    return data['model'], data['label_encoders'], data['le_y'], data['features']

# Prever nova entrada usando modelo salvo
def prever_com_modelo_salvo(entrada, model_file='modelo_arvore.pkl'):
    model, label_encoders, le_y, features = carregar_modelo(model_file)
    entrada_proc = []
...
# Matriz de confusão usando os próprios dados de treinamento
...
def main():
    ...
    json_path = "Dados.json"  # Corrigido o nome do arquivo
    ...
    # Treinar e salvar o modelo
    modelo = treinar_e_salvar_modelo(X, y, label_encoders, le_y, feature_names)
    # Avaliar modelo
    cm, classes = matriz_confusao_treinamento(modelo, X, y, le_y)
    plotar_matriz_confusao(cm, classes)
    # Exemplo de previsão usando o modelo salvo
    entrada = [
        "rosado",  # cor_aspecto
        "visível",  # septo_horizontal
        "ausente",  # perimisio
        "conformação padrão",  # miomeros_mioseptos
        "branca"  # musculatura
    ]
    resultado = prever_com_modelo_salvo(entrada)
    if resultado:
